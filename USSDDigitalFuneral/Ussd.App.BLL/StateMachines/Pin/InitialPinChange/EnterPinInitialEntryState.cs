...
{
    public class EnterPinInitialEntryState : MenuState
    {

        public EnterPinInitialEntryState(MenuState previousState) : base(previousState)
        {

        }

        public EnterPinInitialEntryState(UssdMenu<LanguageDto> handle, LanguageDto language, IDictionary<string, object> valueStash)
        ...
        public override async ValueTask<UssdResponseDTO> ProccessRequest(UssdRequestDTO request)
        {
            ...
            var message = ValueStash["CurrentPinMessage"].ToString();
            menuBuilder.AppendNewLine(message);
            ...
            Handle.CurrentState = new EnterPinInitialExitState(this)
            {
